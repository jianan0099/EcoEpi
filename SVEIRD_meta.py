...
class DiscreteSVEIRD:
    def __init__(self, Re_thre, phi, k, hyper_file_path):
        ...
        self.upsilon = hyper_paras['upsilon']
        self.beta = hyper_paras['R0'] * hyper_paras['alpha']
        self.eta = hyper_paras["eta"]
        self.epsilon = hyper_paras["epsilon"]
        self.nu = hyper_paras['nu']
        self.sigma = hyper_paras["sigma"]
        self.alpha = hyper_paras['alpha']
        self.varepsilon = hyper_paras['varepsilon']
        self.psi = hyper_paras['psi']
    ...
    @staticmethod
    def get_linear_CHI(t, t_keep_time, t_last_known, CHI_last_known):
        slope = -CHI_last_known/(t_keep_time-t_last_known)
        intercept = CHI_last_known/(t_keep_time-t_last_known) * t_keep_time
        return slope * t + intercept

    @staticmethod
    def get_quadratic_CHI_neg(t, t_keep_time, CHI_last_known):
        a = -CHI_last_known / (t_keep_time ** 2)
        return a * t ** 2 + CHI_last_known

    @staticmethod
    def get_quadratic_CHI_pos(t, t_keep_time, CHI_last_known):
        a = CHI_last_known / (t_keep_time ** 2)
        b = - 2 * CHI_last_known / t_keep_time
        return a * t ** 2 + b * t + CHI_last_known
    ...
    def trans_SVEIRD(self, NPI_policy_scenario, keep_time_step):
        ...
        self.unable_to_work_popu.append((IS + IV).tolist())
        ...
        for t in range(1, self.tau+1):
            if NPI_policy_scenario == 'keep_curr':
                if t > keep_time_step:
                    ...
                    CHI_value_t = self.CHI_current_known[:, 0]

            elif NPI_policy_scenario == 'linear':
                if t > keep_time_step:
                    CHI_value_t = np.zeros(self.c_num)
                else:
                    CHI_value_t = self.get_linear_CHI(t, keep_time_step, 0,
                                                    self.CHI_current_known[:, 0])
            elif NPI_policy_scenario == 'quad_neg':
                if t > keep_time_step:
                    CHI_value_t = np.zeros(self.c_num)
                else:
                    CHI_value_t = self.get_quadratic_CHI_neg(t, keep_time_step,
                                                    self.CHI_current_known[:, 0])
            elif NPI_policy_scenario == 'quad_pos':
                if t > keep_time_step:
                    CHI_value_t = np.zeros(self.c_num)
                else:
                    CHI_value_t = self.get_quadratic_CHI_pos(t, keep_time_step,
                                                    self.CHI_current_known[:, 0])
