...
def get_epidemic_data_for_specific_date(save_dir, target_t0_date_owid_, infectious_period_in_days_):
    ...
    npi_info_date_range, date_length = \
        get_npi_stringency_data_range(owid, target_t0_date_owid_)
...
def read_raw_epidemic_data(target_t0_date_owid_, infectious_period_in_days_):
...
def complementary_popu_info():
    ...
    complementary_popu = {
        'ERI': 3645280,  # Eritrea  https://www.worldometers.info/world-population/eritrea-population/
        'GGY': 63590,  # Guernsey https://www.gov.gg/population
        'JEY': 103267,  # Jersey https://www.bbc.com/news/world-europe-jersey-61092967
...
def get_actual_daily_mobility_matrix(comm):
    G_air = nx.read_gexf('raw_data/G_air_2020_complete.gexf')
    update_G_air = nx.DiGraph(G_air.subgraph(comm))
    update_G_air.remove_edges_from(nx.selfloop_edges(update_G_air))
...
def generate_country_info(hyper_file_path, target_t0_date_owid, infectious_period_in_days,
                          CHI_thre_mild, CHI_thre_moderate):
    CHI_date_length = cal_days_between_dates(target_t0_date_owid,target_t0_date_owid)+1
    ...
    fully_vaccinated, total_cases, total_deaths, active_cases, NPI_stringency_index = \
        read_raw_epidemic_data(target_t0_date_owid, infectious_period_in_days)
    ...
    for c in comm_regs:
        ...
        if c in NPI_stringency_index and NPI_stringency_index[c][0] == NPI_stringency_index[c][0]:
            ...
            stringency_index_range_epi[comm_regs.index(c)] = \
                np.array([10] * (cal_days_between_dates(target_t0_date_owid,target_t0_date_owid)+1))
            stringency_info[c_index] = 'mild'
    country_info = {
        'common': comm_regs,
...
def read_country_info(hyper_file_path, target_t0_date_owid, infectious_period_in_days,
                      CHI_thre_mild, CHI_thre_moderate):
    save_path = hyper_file_path + '/clean_epidemic_data.pkl'
