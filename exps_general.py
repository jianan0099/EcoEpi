...
class FinalExps:
    ...
    def specify_timing(self, rho=0.04, varphi=0.8, timing_range=range(52 * 7 * 1, 52 * 7 * 2 + 4 * 7, 4 * 7)):
    ...
    def main_results(self, rho=0.04, varphi=0.8):
        main_results_file = self.eco_epi_model.hyper_matlab_results_path + '/' \
                            + 'main_rho_' + str(rho) + '_varphi_' + str(varphi)
        if not os.path.exists(main_results_file):
            os.mkdir(main_results_file)
    ...
    def market_structure_change(self, rho=0.04, varphi=0.8):
    ...
    def competition_explain(self, rho=0.04, varphi=0.8):
        ...
        for keep_time_step in [52 * 7 * 1, 52 * 7 * 1.5, 52 * 7 * 2]:
            ...
            for sec in sector_list:
                ...
                sector_ave_profit = \
                    np.sum(
                        np.array(eco_epi_data['profit'])[:, self.eco_epi_model.product_index_list_in_items(sec_index)] *
                        np.array(eco_epi_data['production_distribution'])[:,
                        self.eco_epi_model.product_index_list_in_items(sec_index)], axis=1)

                target_region_list_index = np.argsort(reshape_init_x[:, sec_index])[::-1][:5]
                if keep_time_step == 52 * 7 * 1:
                    ...
                    CHI_scores[sec] = [self.economic_region_chi_value[self.eco_epi_model.regions[reg_index].upper()] for
                                       reg_index
                                       in target_region_list_index]
                    CHI_levels[sec] = [
                        self.eco_epi_model.get_eco_region_CHI_class(self.economic_region_npi_policy, reg_index) for
    ...
    def all_experiments(self, rho=0.04, varphi=0.8):
...
exp_supple_change_phi.all_experiments(rho=0.04, varphi=0.8)
exp_supple_change_phi.specify_timing(rho=0.04, varphi=0.8, timing_range=range(52 * 7 * 1, 52 * 2 * 7 + 4 * 7, 4 * 7))
...
# rho=0.025
exp_supple_change_phi = FinalExps()
exp_supple_change_phi.main_results(rho=0.025)
# rho=0.03
exp_supple_change_phi = FinalExps()
exp_supple_change_phi.main_results(rho=0.03)
# rho=0.06
exp_supple_change_phi = FinalExps()
exp_supple_change_phi.main_results(rho=0.06)

# change varphi
# varphi=0.5
exp_supple_change_phi = FinalExps()
exp_supple_change_phi.main_results(varphi=0.5)
# varphi=0.9
exp_supple_change_phi = FinalExps()
