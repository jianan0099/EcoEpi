function com_explain_generalize(raw_epidemic_data_info, I_thre, Re_thre, phi, k, CHI_thre, rho, ...
    varphi, if_base, supple_name, year_info, year_info_NPI)
% ------- basic situation -------------------------
...
keep_weeks_info = [52*1, 52*1.5, 52*1.5];
specific_year = 1;
...
for row=1:1
     for col=1:3
        ...
        if col==1
             title({'Manufacture of', 'electrical equipment'},'FontWeight','normal','Units', 'normalized', 'Position', [0.5, 1.02, 0]); % 
         end
         
         if col==2
             title({'Wholesale and', 'retail trade'},'FontWeight','normal','Units', 'normalized' , 'Position', [0.5, 1.02, 0]); %
         end
         
         if col==3
             title({'Education'},'FontWeight','normal','Units', 'normalized', 'Position', [0.5, 1.02, 0]);
         end
...
for row=3:4
    metric = metrics(row-1);
     ...
     for col=1:3
         ...
         for i=0:4
            base_scenario_key = strcat(rho, '_', varphi, '_', year_info_NPI(specific_year));
            line_info = string(strcat(base_scenario_key, year_info(specific_year),'_', sector, '_', string(i), metric)); 
         ...
         if row==3
            legend(h_all,largest_producer, 'Units', 'normalized','FontSize',font_size-3, 'NumColumns', 2, 'Position', [0.215+(col-1) * 0.32,0.4, 0.1, 0.06])
            legend boxoff 
         end
        text(-0.2, 1.2, texts(figs), 'Units', 'Normalized','FontSize',font_size,'FontWeight','bold');
        ...
        xlim([1 260/point_length])
        set(gca, 'XTick', linspace(1, 260/point_length, 6), 'XTickLabel',{'0','1','2','3','4','5'},'FontSize',font_size, 'Layer', 'top')
        box on
