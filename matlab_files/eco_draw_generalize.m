function eco_draw_generalize(raw_epidemic_data_info, I_thre, Re_thre, phi, k, CHI_thre, rho, ...
    varphi, y_lim_min, y_lim_max, if_base, supple_name, legend_position,col_info,col_info_NPI)
...
point_length = 3; % plot every [point_length] points
texts = char(97:120);
point_interval = 2;
marker_size = 2;
line_width = 2;
...
reopen_text_x = [0.23, 0.313, 0.313];
...
for row=1:4
     for col=1:3
        ...
        base_scenario_key = strcat(rho, '_', varphi, '_', col_info_NPI(col));
       ...
       total_loss_name_specific = '_ave_tot_GVA_';
        ...
        if (col==1 && row==1)||(col==1 && row==3)||(col==1 && row==2)
            rotation_text_reopen = 0;
            y_text_reopen = 0.9072;
        end

        if col==1 && row==4
            text(0.3,y_text_reopen,0, {'Full', 'reopening'},'FontSize',font_size-2, 'color',[82/255,82/255,82/255],'Rotation',rotation_text_reopen, ...
                 'Units','normalized');
        else
        ...
        title(row_data(row), 'FontSize',font_size,'FontWeight','normal')
        ...
        if col==1
            legend(h_all,'Total','Pandemic','Supply-chain', 'Competition', 'Units', 'normalized',...
                'Position', [legend_position(1)+(col-1) * 0.32, legend_position(2)-(row-1)* 0.243, legend_position(3), legend_position(4)],'FontSize',font_size-3)
            legend boxoff 
        end 
    end
end
