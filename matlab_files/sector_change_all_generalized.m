function sector_change_all_generalized(raw_epidemic_data_info, I_thre, Re_thre, phi, k, CHI_thre, rho, varphi, year_info, year_info_NPI)
% sector change all
font_size = 11;
% ------- basic situation -------------------------
...
reopen_scenarios = {'Explosive reopening', 'Steady reopening', 'Incremental reopening'};
...
for keep_year_index=1:3
    keep_year = year_info(keep_year_index);
    NPI_info = year_info_NPI(keep_year_index);
for part_num=1:2
    
figure_save_path = strcat('results/', raw_epidemic_data_info, '/supple_figs_summary/full_market_change_results',...
    keep_year, '_', NPI_info, time_stamp,'_', string(part_num), '.eps');
...
base_scenario_key = strcat(rho, '_', varphi, '_', NPI_info);
