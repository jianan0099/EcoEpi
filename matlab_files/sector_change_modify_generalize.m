function sector_change_modify_generalize(raw_epidemic_data_info, I_thre, Re_thre, phi, k, CHI_thre, rho, ...
    varphi, if_base, supple_name,col_info, col_info_NPI)
...
% --------- plot settings --------------------------
row_info = {'', '_re', ''};
titles = {'Explosive reopening', 'Steady reopening', 'Incremental reopening'};
texts = char(97:120);   
font_size = 15;
...
for row=1:1
    for col=1:3
        ...
        title(titles(col),'FontSize',font_size,'FontWeight','normal','Units', 'normalized', 'Position', [0.5,1.1,0])
